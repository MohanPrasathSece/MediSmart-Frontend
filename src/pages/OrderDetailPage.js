            ...
            <div className="flex items-center justify-between">
              ...
              {isDeliveryBoy && (
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-white text-primary-700 px-3 py-2 rounded-md font-semibold hover:bg-primary-50"
                >
              ...
              <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <ul className="space-y-3">
                  {order.items?.map((item, idx) => {
                    const keyCandidate = (item?.medicine && typeof item.medicine === 'object') ? (item.medicine._id || item.medicine.id) : item.medicine;
                    const key = keyCandidate || `${order._id}-item-${idx}`;
                    const rawName = typeof item.name === 'string' ? item.name.trim() : '';
                    const placeholderRegex = /^item(\s*#?\d*)?$/i;
                    const isPlaceholder = rawName === '' || placeholderRegex.test(rawName);
                    const medName = (item?.medicine && typeof item.medicine === 'object') ? item.medicine.name : undefined;
                    const fallbackAlt = item.medicineName || item.displayName || undefined;
                    const name = medName || (!isPlaceholder && rawName) || fallbackAlt || 'Unknown medicine';
                    if (!medName && !fallbackAlt && isPlaceholder) {
                      // Hide entries that are just placeholder without any real name
                      return null;
                    }
                    const qty = item.quantity ?? 1;
                    const price = Number(item.finalPrice || item.price || 0);
                    return (
                      <li key={key} className="flex justify-between items-center text-secondary-700">
                        <span>{name} <span className="text-sm text-secondary-500">x {qty}</span></span>
