                  ...
                  <div className="text-xs text-secondary-700">
                    {(() => {
                      const names = (order.items || []).map(i => {
                        const raw = (typeof i?.name === 'string') ? i.name.trim() : '';
                        const isPlaceholder = raw === '' || /^item(\s*#?\d*)?$/i.test(raw);
                        const medName = (i?.medicine && typeof i.medicine === 'object') ? i.medicine.name : undefined;
                        return medName || (!isPlaceholder ? raw : null);
                      }).filter(Boolean).slice(0,3);
                      return `Items: ${names.length > 0 ? names.join(', ') : `${order.items?.length||0} item(s)`}`;
                    })()}
                  </div>
