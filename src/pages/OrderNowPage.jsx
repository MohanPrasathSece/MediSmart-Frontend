...
import { Pill, Building, Package, IndianRupee, MapPin } from 'lucide-react';
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow p-5">
          <div className="flex gap-5">
            {(() => {
              ...
              const src = (typeof primaryFromImages === 'string' && /^https?:\/\//i.test(primaryFromImages))
                ? primaryFromImages
                : ((typeof medicine.imageUrl === 'string' && /^https?:\/\//i.test(medicine.imageUrl)) ? medicine.imageUrl : null);
              if (src) {
                return (
                  <img
                    src={src}
              ...
              return (
                <div className="w-40 h-28 bg-secondary-100 rounded flex items-center justify-center">
                  <span className="text-secondary-500 font-semibold">{initials}</span>
                </div>
              );
            })()}
            <div className="flex-1">
              ...
              {typeof medicine.price === 'number' && (
                <p className="text-lg font-bold text-primary-600 mt-2 flex items-center gap-1">
                  <IndianRupee size={18} /> ₹{medicine.price.toFixed(2)}
                </p>
              )}
              {/* Stock levels hidden from patient view */}
            </div>
          </div>
